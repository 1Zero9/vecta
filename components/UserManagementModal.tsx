...
export function UserManagementModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
      <div className="bg-slate-900 border border-white/15 rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl space-y-6 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-4 rounded-2xl bg-slate-950/70 border border-white/10 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-cyan-500 to-emerald-400 text-slate-950 font-black text-lg flex items-center justify-center shadow-lg shadow-emerald-500/20">
              {currentUser.avatar}
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                {currentUser.isDemo && (
                  <span className="px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 text-[10px] font-mono font-bold border border-emerald-500/30 uppercase">
                    Verified Default Account
                  </span>
                )}
              </div>
              <div className="text-xs text-slate-400 mt-0.5">
                {currentUser.email} • <span className="text-slate-300 font-semibold">{currentUser.role}</span>
              </div>
            </div>
          </div>

          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-950/50 border border-emerald-500/30 text-emerald-400 text-xs font-mono font-bold">
        ...
        {!isCreatingCustom ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="w-7 h-7 rounded-xl bg-cyan-500/20 text-cyan-400 text-xs font-bold flex items-center justify-center">
                    AM
                  </span>
                  <Sparkles className="w-3.5 h-3.5 text-cyan-400" />
                </div>
                <h5 className="font-bold text-white text-xs">Alex Mercer</h5>
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="w-7 h-7 rounded-xl bg-amber-500/20 text-amber-400 text-xs font-bold flex items-center justify-center">
                    EB
                  </span>
                  <ShieldCheck className="w-3.5 h-3.5 text-amber-400" />
                </div>
                <h5 className="font-bold text-white text-xs">Elena Beaumont</h5>
                <p className="text-[11px] text-slate-400 mt-0.5 line-clamp-2">
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="w-7 h-7 rounded-xl bg-indigo-500/20 text-indigo-400 text-xs font-bold flex items-center justify-center">
                    MS
                  </span>
                  <Cpu className="w-3.5 h-3.5 text-indigo-400" />
                </div>
                <h5 className="font-bold text-white text-xs">Marcus Sterling</h5>
                <p className="text-[11px] text-slate-400 mt-0.5 line-clamp-2">
          ...
          <form onSubmit={handleCustomSubmit} className="space-y-4 text-xs p-4 rounded-2xl bg-slate-950/60 border border-white/10">
            <div className="flex items-center justify-between pb-2 border-b border-white/10">
              <span className="font-bold text-white text-sm">Create New Account</span>
              <button
                type="button"
