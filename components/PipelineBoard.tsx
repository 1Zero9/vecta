...
const STAGES: { id: ApplicationStage; label: string; color: string; border: string }[] = [
  { id: "saved", label: "Saved / Evaluating", color: "bg-slate-800 text-slate-300", border: "border-slate-700" },
  { id: "drafting", label: "Drafting & Tailoring", color: "bg-cyan-950/60 text-cyan-400", border: "border-cyan-500/30" },
  { id: "applied", label: "Applied / Submitted", color: "bg-blue-950/60 text-blue-400", border: "border-blue-500/30" },
  { id: "screening", label: "Recruiter Screen", color: "bg-indigo-950/60 text-indigo-400", border: "border-indigo-500/30" },
  { id: "interviewing", label: "Technical Interview", color: "bg-amber-950/60 text-amber-400", border: "border-amber-500/30" },
  { id: "offer", label: "Offer / Negotiation", color: "bg-emerald-950/60 text-emerald-400", border: "border-emerald-500/40" },
];
...
export function PipelineBoard({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Pipeline Toolbar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 glass-panel rounded-3xl p-6 border border-white/10 shadow-lg">
        <div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 font-bold text-xs shadow-md shadow-emerald-500/20 transition-all"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 overflow-x-auto pb-4">
        {STAGES.map((col) => {
          ...
          return (
            <div
              key={col.id}
              className="bg-slate-950/60 rounded-3xl p-3 sm:p-4 border border-white/10 flex flex-col min-h-[500px] shadow-sm"
            >
              {/* Stage Header */}
              ...
              <div className="space-y-3 flex-1 overflow-y-auto pr-1">
                {items.length === 0 ? (
                  ...
                  items.map((app) => (
                    <div
                      key={app.id}
                      className="p-3.5 rounded-2xl bg-slate-900/90 border border-white/10 hover:border-emerald-500/40 transition-all space-y-2.5 shadow-md group relative"
                    >
                      ...
                      {editingNotesId === app.id ? (
                        <div className="space-y-1.5 pt-1">
                          <textarea
                            ...
                            rows={2}
                            className="w-full bg-slate-950 border border-white/20 rounded-xl p-2 text-xs text-slate-200 focus:outline-none focus:border-emerald-500"
                        ...
                        <div
                          ...
                          className="p-2 rounded-xl bg-slate-950/60 border border-white/5 text-[11px] text-slate-400 hover:text-slate-200 cursor-pointer italic"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-slate-900 border border-white/15 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <h3 className="font-bold text-white text-base">Add Tracked Application</h3>
