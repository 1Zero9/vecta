...
export function ProfileDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/70 backdrop-blur-sm">
      <div className="bg-slate-900 border-l border-white/10 max-w-xl w-full h-full flex flex-col shadow-2xl animate-in slide-in-from-right duration-300">
        
        {/* Drawer Header */}
        <div className="flex items-center justify-between p-6 border-b border-white/10 bg-slate-950/80">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-cyan-500 to-emerald-400 text-slate-950 font-black flex items-center justify-center shadow-md">
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-5 flex-1 text-xs">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-slate-950 border border-white/10 rounded-xl p-2.5 text-white"
              />
              <button
                type="button"
                onClick={handleAddSkill}
                className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold"
              >
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              {formData.skills.map((skill) => (
                <span
                  key={skill}
                  className="px-2.5 py-1 rounded-lg bg-slate-800 text-slate-200 flex items-center gap-1.5 border border-white/5"
                >
                  <span>{skill}</span>
                  <button
                    type="button"
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              {formData.certifications.map((cert) => (
                <span
                  key={cert}
                  className="px-2.5 py-1 rounded-lg bg-amber-500/10 text-amber-300 flex items-center gap-1.5 border border-amber-500/20"
                >
                  <span>{cert}</span>
                  <button
                    type="button"
          ...
          <div className="space-y-1.5">
            <label className="block text-slate-300 font-bold uppercase tracking-wider flex items-center justify-between">
              <span>Resume & Career History (For Deep ATS Keyword Parsing)</span>
              <span className="text-[10px] text-emerald-400 font-mono">Real-time vector synced</span>
            </label>
            <textarea
              rows={6}
              ...
              className="w-full bg-slate-950 border border-white/10 rounded-2xl p-3 text-slate-300 font-mono text-xs focus:outline-none focus:border-emerald-500 leading-relaxed"
            />
          </div>
          ...
          <div className="pt-4 border-t border-white/10 flex items-center justify-between">
            ...
            <button
              type="submit"
              className="px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 font-bold rounded-xl flex items-center gap-2 shadow-md shadow-emerald-500/20 transition-all"
            >
              {savedSuccess ? (
                <>
