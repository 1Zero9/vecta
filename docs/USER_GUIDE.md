...
The current milestone is **Vecta v0.11.0 Preview**, using skill taxonomy **v1.1.0** and export schema **v1**. These identifiers are shown in the interface and included in exports so a result can be traced to the rules that produced it.
...
This is a local prototype profile switcher, not a production account-management system. Every profile represents a person searching for roles; Vecta has no recruiter or vacancy-publisher account type. Authentication, session and device management, account recovery, and administrator support tools remain roadmap work.
