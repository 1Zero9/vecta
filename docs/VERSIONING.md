...
| --- | --- | --- | --- |
| Product | `0.15.0` | `package.json` | The complete Vecta application milestone. |
...
- `0.15.0` — Authenticated D1 application pipeline snapshots with notes, activity history, intentional empty-pipeline persistence, and confirmed conflict resolution.
...
- `0.10.0` — Sites-compatible vinext/Vite/Cloudflare Worker delivery, with an explicit worker-safe local-first persistence boundary and documented build path.
