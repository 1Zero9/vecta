...
The Sites adapter intentionally disables native Prisma/SQLite synchronization. Its authenticated account and protected-profile routes use the `DB` D1 binding; saved roles and pipeline state remain in browser storage.
...
- Automated unit and component suite: 71 tests across 21 files, passing.
...
| Hosted legacy `/api/user` or consent sync returns `offline: true` | Expected in v0.13.0: those Prisma adapters remain disabled. Use `/api/profile` for the explicitly protected D1 profile; saved roles, pipeline, and consent are still browser-local. |
